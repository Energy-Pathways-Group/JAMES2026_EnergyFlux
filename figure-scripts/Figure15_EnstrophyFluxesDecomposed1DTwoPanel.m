...
runNumber=18;
wvdArray{1} = WVDiagnostics(basedir + replace(getRunParameters(runNumber),"256","256") + ".nc");
timeIndicesArray{1} = 2801:3001;
...
for iWVD = 1:2
    ...
    % text label
    if iWVD==1
        ylabel(gca,{"256^2 \times 43","potential enstrophy flux (" + wvd.z_flux_scale_units + ")"})
        set(gca,'XTickLabels',[])
        text(ax,max(xlim)*.95,max(ylim),'a)','FontSize',14,'HorizontalAlignment','left','VerticalAlignment','top')
    else
        lgd1 = legend('location','southwest');
        ylabel(gca,{"512^2 \times 86","potential enstrophy flux (" + wvd.z_flux_scale_units + ")"})
        xlabel(gca,"wavelength (km)");
