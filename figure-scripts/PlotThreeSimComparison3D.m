...
function fig = PlotThreeSimComparison3D(wvd1,wvd9,wvd18,options)
arguments
    wvd1
    wvd9
    wvd18
...
if ~isfield(options,"yForXZSlice3")
    iY3 = round(wvt.Nx/2);
else
    iY3 = round(options.yForXZSlice3/(wvt.y(2)-wvt.y(1)));
end

% nColumns = 3; nRows = 1;
nColumns = 1; nRows = 3;
if nColumns == 3 && nRows==1
    figPos = [50 50 950 300];
elseif nColumns ==1 && nRows==3
    figPos = [50 50 400 800];
else
...
% ylabel('distance (km)')
zlabel('depth (km)')
title(ax, "hydrostatic: geostrophic")

ax = nexttile(tl,2);
makeVorticityXYZPlot(wvd9.wvt.zeta_z,iY2);
xticklabels([])
yticklabels([])
zticklabels([])
title(ax, "hydrostatic: geostrophic + wave")

ax = nexttile(tl,3);
makeVorticityXYZPlot(wvd18.wvt.zeta_z,iY3);
xticklabels([])
yticklabels([])
zticklabels([])
title(ax, "non-hydrostatic: geostrophic + wave")
if nRows==1
